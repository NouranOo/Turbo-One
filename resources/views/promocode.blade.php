...
<div class="modal fade" id="block_user{{$promocode->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
<div class="modal-dialog" role="document">
    <form class="modal-content text-center">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Do you want active promo code ?</h5>
        </div>
        <div class="modal-footer text-center">
        <a href="{{ route('activepromo', $promocode->id) }}"> <button type="button" class="custom-btn red-bc">OK</button></a>
...
<div class="modal fade" id="unblock_user{{$promocode->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form class="modal-content text-center">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Do you want disactive promo code ?</h5>
            </div>
            <div class="modal-footer text-center">
            <a href="{{route('disactivepromo', $promocode->id) }}"> <button type="button" class="custom-btn red-bc">OK</button></a>
                <button type="button" class="custom-btn green-bc" data-dismiss="modal">close</button>
            </div>
        </form>
    </div>
  ...
  <div class="content">
                        ...
                        <div class="widget">
                                <div class="widget-content">
                                    <div class="col-sm-12">
                                        <a href="{{route('addpromocode')}}" class="custom-btn green-bc">add new promo code</a>
                                    </div>
                                    <div class="spacer-15"></div>
                                        ...
                                        <table id="datatable"  class="table table-bordered table-hover text-center">
                                            ...
                                            @foreach($promocodes as $promocode)
                                                <tr>
                                                    ...
                                                    <td>
                                                      {{$promocode->DiscountType->Type}}
                                                    </td>
                                                 
                                                    <td class="text-center">
                                                        <a href="{{route('editpromocode',[$promocode->id])}}" class="icon-btn green-bc">
                                                            <i class="fa fa-pencil"></i>
                                                            edit
                                                        </a>
                                                       
                                                    @if($promocode->IsActive != 1)
    <button data-url="{{route('activepromo',['id' => $promocode->id])}}" data-toggle="modal" data-target="#block_user{{$promocode->id}}" class="icon-btn red-bc ">
                                     
                                   Active
                                 </button>
                                @else
                                 <button data-url="{{route('disactivepromo',['id' => $promocode->id])}}" data-toggle="modal" data-target="#unblock_user{{$promocode->id}}" class="icon-btn red-bc ">
